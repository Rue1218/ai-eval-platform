...
def test_image_attachment_is_projected_as_internal_image_part(tmp_path: Path) -> None:
    """图片附件应保留图片内容块，供两类协议适配器转换后发送给视觉模型。"""
...
def test_image_parts_are_adapted_for_anthropic_and_openai_chat() -> None:
    """内部图片块应映射为 Anthropic 与 OpenAI Chat 各自的内容结构。"""
    content = [
        {"type": "text", "text": "看图"},
    ...
    openai_chat = _adapt_message_content(content, "openai_chat")

    assert anthropic[0] == {"type": "text", "text": "看图"}
    assert anthropic[1]["type"] == "image"
    assert anthropic[1]["source"]["data"] == "ZmFrZQ=="
    assert openai_chat[0] == {"type": "text", "text": "看图"}
    assert openai_chat[1] == {
        "type": "image_url",
