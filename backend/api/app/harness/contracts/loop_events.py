...
EVENT_SCHEMAS: dict[str, dict[str, Any]] = {
    ...
    "assistant/attempt_start": _event(
        "A provider streaming attempt started.",
        {
            ...
            "history_selection": _field(_object_schema({
                "algorithm": {"type": "string", "enum": ["message_indices.v1", "message_indices.v2"]},
                "indices": {"type": "array", "items": {"type": "integer", "minimum": 0},
                            "uniqueItems": True},
                "message_count": {"type": "integer", "minimum": 0},
                "input_fingerprint": {"type": "string", "pattern": "^[0-9a-f]{64}$"},
                "transformations": {"type": "array", "items": _object_schema({
                    "index": {"type": "integer", "minimum": 0},
                    "removed_fields": {"type": "array", "items": {"const": "protocol_state"}},
                    "reason": {"const": "model_compatibility"},
                }, required=("index", "removed_fields", "reason"))},
    ...
    "assistant/message": _event(
        "Completed or interrupted assistant message committed to history.",
        {
            ...
            "latency_ms": _field(_INTEGER, "本次模型请求从流建立到终态的单调时钟耗时（毫秒）。"),
        },
        required=("turn", "step", "attempt_id"),
    ),
    "assistant/attempt": _event(
        "A provider attempt failed before a message could be committed.",
        {
            ...
            "latency_ms": _field(_INTEGER, "失败前本次模型请求的单调时钟耗时（毫秒）。"),
        },
    ...
    "tool/dispatch": _event(
        "The tool crossed the execution boundary and may have side effects.",
        {
            ...
            "registry_name": _field(_STRING, "Authorized registry name after alias resolution."),
            "wire_name": _field(_STRING, "Model-visible tool name."),
            "tool_contract_version": _field(_STRING, "Normalized tool argument contract version."),
            "execution_id": _field(_STRING, "Stable execution-boundary identifier."),
        },
        required=("turn", "step", "attempt_id", "call_id", "name"),
    ),
    "tool/result": _event(
