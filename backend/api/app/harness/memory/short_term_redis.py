...
class RedisMemoryPort(MemoryPort):
    ...
    @staticmethod
    def _prefix(query: MemoryQuery) -> str:
        """构造作用域键前缀，不把消息正文或密钥写入 Redis key。"""
        return f"harness:memory:tenant:{query.tenant_id}:user:{query.user_id}:session:{query.session_id}"

    def _records_key(self, query: MemoryQuery, trace_id: str) -> str:
        """每回合记录键必须含 trace_id，满足审计与失效隔离要求。"""
        return f"{self._prefix(query)}:trace:{trace_id}:records"

    def _index_key(self, query: MemoryQuery) -> str:
        """会话索引只保存 trace_id，供后续回合跨 trace 召回短期记录。"""
        return f"{self._prefix(query)}:traces"

    def _failure(self, exc: Exception) -> AppError:
    ...
    def _retrieve_sync(self, query: MemoryQuery) -> list[MemoryRecord]:
        ...
        for trace_id in sorted(str(item) for item in self._client.smembers(self._index_key(query)) or set()):
    ...
    async def append(self, record: MemoryRecord, *, trace: TraceContext) -> None:
        ...
        validate_memory_query(query, trace)
        normalized = record.model_copy(update={"origin_trace_id": record.origin_trace_id or trace.trace_id, "origin_span_id": record.origin_span_id or trace.span_id})
        try:
    ...
    async def forget(self, source_id: str, *, trace: TraceContext) -> None:
        """扫描本模块 records 键并重写受影响列表，撤权后不允许继续命中缓存。"""
    ...
    async def put_idempotency_key(self, key: str, *, trace: TraceContext) -> None:
        ...
        try:
            await asyncio.to_thread(self._client.set, f"harness:memory:idempotency:trace:{trace.trace_id}:{digest}", "1", ex=self._retention.normalized_ttl())
        except Exception as exc:
            raise self._failure(exc) from exc
