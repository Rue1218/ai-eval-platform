...
def build_import_template() -> bytes:
    ...
    help_lines = [
        ...
        "2. 平台标准列：用例编号、策略、优先级、模块、子模块、功能点、用例名称、前置条件、步骤、预期结果、测试类型。",
        "3. 亦支持 testcase-tools 标准列：用例编号、所属模块、用例标题、优先级、用例类型、前置条件、测试步骤、预期结果。",
        "4. 简化列只需：模块、名称、步骤、预期；缺策略时默认「正向」，缺优先级时默认 FHX。",
        "5. 用例名称必填；空行跳过。表头可出现在前 20 行（允许首行写标题）。",
        "6. 策略取值：正向 / 反向 / 边界 / 等价类 / 状态迁移 / 场景；优先级 HX/FHX/BJ/YC/ZD/BL（HX核心、FHX非核心、BJ边界问题、YC异常、ZD中断、BL遍历）。",
