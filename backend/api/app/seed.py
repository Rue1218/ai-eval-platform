...
def _seed_cases(db: Session, admin: User) -> CaseSet | None:
    ...
    db.flush()
    items = [
        ("功能正向", "P0", "收银台", "余额充足场景下完成支付", "用户已登录且账户余额 ≥ 订单金额", "1. 提交订单\n2. 选择余额支付\n3. 确认支付", "支付成功，生成支付流水号，订单状态变更为「已支付」", "功能"),
        ("边界值", "P1", "收银台", "支付金额等于账户余额", "账户余额恰好等于订单金额", "1. 提交订单\n2. 余额支付", "支付成功，账户余额扣减为 0", "功能"),
        ("异常容错", "P1", "收银台", "支付网关超时重试", "Mock 网关首次响应超时", "1. 发起支付\n2. 网关超时\n3. 系统自动重试", "重试成功后支付完成，不产生重复扣款", "异常"),
        ("性能效率", "P2", "收银台", "高峰期支付接口响应", "并发 100 QPS 持续 5 分钟", "1. 压测引擎发压\n2. 观察 P99 延迟", "P99 ≤ 800ms，错误率 ≤ 0.5%", "性能"),
        ("安全合规", "P1", "收银台", "支付密码连续错误锁定", "同一账户连续输错密码", "1. 连续 5 次输入错误支付密码", "账户支付功能锁定 30 分钟并触发风控告警", "安全"),
        ("兼容性", "P2", "收银台", "主流浏览器支付流程", "覆盖 Chrome / Edge / Safari 最新版", "1. 分别在各浏览器完成一笔支付", "各浏览器支付流程一致且无样式错乱", "兼容"),
    ]
    for idx, (strategy, priority, module, name, precondition, steps, expected, test_type) in enumerate(items):
        db.add(
            CaseItem(
