...
| Agent 图 | **骨架化 + H1 Router**：开关关闭为单节点纯对话（`START → chat_stream → END`）；`hybrid_engine_enabled=True` 时接入顶层 `router` 节点（L0/L1 分流写 `engine` 审计，四路引擎当前统一降级 `chat_stream`）；无工具/确认卡/澄清卡/斜杠。H0 基础设施（缓存分段 + AgentRegistry + L2 指令槽）与 H1 Router（2026-09-03）已就绪，主开关默认关闭 | `backend/api/app/agent/graph.py`、`routing.py` |
