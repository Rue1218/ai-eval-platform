...
| 上游权威 | Harness 需求文档 V1.4.4 §4.1、§2.4、§7、§9；API.md V1.4+；PRD §5.1.2 |
...
| 文件 | 操作 | 作用 |
| :--- | :--- | :--- |
| `docs/AI测试与评估平台-Harness-提示词工程层.md` | 新增 V0.2 → 修订 V0.3 | V0.2 M1 提示词工程层模块设计：定义系统策略五段、四阶段严格 JSON 输出协议、注入测试与边界守护，含接口签名级；V0.3 开放问题闭环：文案阶段 1 PR 内评审、`SystemVars` 最小集+扩展点、协议版本严格不兼容、注入测试集 CI 管控、`CompactProtocol` 移归 M2（本层只留规划/ReAct/复核三协议）。 |
