...
| 版本 | V1.1 |
| 日期 | 2026-08-19 |
| 最近修订 | 2026-08-20：落地模块 7（AGT-UI-01/02/05/08、AGT-CTX-02、AGT-SLH-01：卡片规范、Markdown 渲染、斜杠面板、ContextMeter、live 清理） |
...
| 时限 | 单次模型 30s；整回合墙钟 **120s** 必须结束（`TIMEOUT` + thought「本轮超时，未出确认卡」） | 单样本 `run.timeout_s`；用例生成 5 分钟；压测 `duration_s`；用例待确认 72h |
