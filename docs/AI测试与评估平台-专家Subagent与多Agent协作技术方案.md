...
> 版本：V0.7 ｜ 审查日期：2026-09-16 ｜ 状态：P1 四项审查修复已实现：开关回退、取消锁存、分批状态和最终预算；真实模型试点和 P2/P3 仍待完成，见 §22。
>
...
| --- | --- | --- |
| 通用助手 | 已有 | 默认协调和通用分析 |
| 测试用例设计专家 | 已有 | 根据需求设计用例并交付结构化成果 |
| 需求分析专家 | 拟新增 | 需求拆分、歧义与验收条件整理 |
| 评测分析专家 | 拟新增 | 评测方案、报告分析与失败定位 |
| 评分复核专家 | 拟新增 | 解释评分分歧、核对证据并形成复核意见 |

P1 先用已有两个角色验证真实委派，必要时同一用例专家启动多个实例。新增角色必须先登记产品范围和工具上限，避免一次性定义没有验证过的完整专家团队。
