...
> 版本：V0.8 ｜ 审查日期：2026-09-16 ｜ 状态：P1 调度与审查修复已实现；本次补齐专家协作接入基准测试的设计，结构化交付、计划交接和新增角色未实现；真实模型试点和 P2/P3 仍待完成。
>
> 用户目标：将平台专家升级为能够独立运行、自由调度、相互通讯和协作的 subagent，并适配现有被测模型、Agent 模型和评分模型。
>
> 本次核对基线：本地仓库 `1cf154b`；已实现契约 PRD V1.29、API V2.7；新会话使用 AgentLoop v2。最初设计调研基线为 `3ac0ce738f4b551510fa5a9212ab0164239390ef`。本次只优化文档，未核验生产部署或调用真实模型。
>
...
### 13.8 专家结构化交付物与受控材料（拟议）

使用待实现的 `expert_deliverable.v1`，字段单一来源为 [API V2.8](AI测试与评估平台-API.md)。它属于评测准备/分析制品，不替换 §8 的消息信封或 `AgentRun.result`，也不把 `run_id` 当作 `task_id`。
...
**当前 P1 的实际边界：** `output_contract` 是提示文本，`agent.result` 返回成果正文和运行结果，没有上述制品发布/校验能力；`agent.spawn` 也不接受 `input_refs`。主 Agent 目前只可在目标中转述获准且长度允许的材料，不能把要求输出 JSON 等同服务端验收完成。

**受控引用的下一步实现：** 服务端从获准引用读取固定版本，校验会话身份、用途和阶段，再将必要材料投影到子运行可读输入区域；子运行写入独立输出区域。不得向模型传任意服务器路径，不把整个父目录挂给专家，也不通过放宽 `.subagents/<run_id>` 边界实现共享。父/兄弟运行读取成果必须经同样的引用校验，禁止为便利复制隐藏答案到通用共享目录。
