...
> V1.66（2026-09-03）：混合驱动引擎 H1 Router 分流审计。`response.completed` payload 新增三个**可选**字段 `engine` / `router_confidence` / `router_reason`（仅在 `HYBRID_ENGINE_ENABLED=true` 时出现；关闭时与骨架化旧客户端字节兼容，无新增 WS 事件名）。`engine ∈ {direct, chat, workflow, agent}` 为顶层分流结果，`router_reason` 为平台生成脱敏中文理由（不含用户原文）；H1 阶段 `workflow`/`agent` 引擎先安全降级至 `chat_stream` 执行并保留审计痕迹。新增只读目录 `GET /api/agents`（§3.6.x）：返回 AgentRegistry 全部 Worker 能力元数据（`agent_id/display_name/capabilities/allowed_tools/max_permission/model_profile_id/description`），仅表示已注册基础设施，不代表模型正在派发子代理（discover 于 H3 恢复）。新增配置项：`HYBRID_ENGINE_ENABLED`（默认 false）、`HYBRID_ROUTER_COT_ENABLED`（默认 false）、`HYBRID_ROUTER_CONFIDENCE_THRESHOLD`（默认 0.7）。
>
...
| `response.completed` | `{finish_reason:"stop",role:"assistant",engine?,router_confidence?,router_reason?}`；持久化并可回放。混合引擎开关（`HYBRID_ENGINE_ENABLED`）开启时附带 H1 分流审计三件套：`engine ∈ {direct,chat,workflow,agent}`、`router_confidence`（0~1）、`router_reason`（平台生成脱敏中文理由）；开关关闭时字段缺席，旧客户端兼容 | 结束流式状态 |
