...
| --- | --- |
| 文档版本 | V1.86 |
| 当前增量 | V1.86：AgentLoop `request_summary.context_meter` 新增系统提示词、Skill、MCP、原生工具、对话消息五类实际输入 token 明细。 |
...
>
> V1.86（2026-09-09）：`assistant.start.data.request_summary.context_meter` 增量提供系统提示词、Skill、MCP、原生工具和对话消息的实际输入 token。它们都由本轮协议序列化请求计量，五项合计等于 `input_tokens`；未注入的来源如实为 0，输出预留不参与前端「已用」比例。
