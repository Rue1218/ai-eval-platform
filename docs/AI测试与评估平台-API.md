...
用户工作区 = 用户自管数据域（目录 `data/workspaces/<id>`，与 legacy 会话目录同根共存）；会话绑定接线随 F3（本期会话不产生绑定）。鉴权：仅属主；写入均写 AuditLog（`workspace_create/rename/delete/purge/folder_create`、解绑 `workspace_unbind` 随 purge）。
