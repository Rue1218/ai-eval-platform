...
> **V1.96（2026-09-14）**：Agent 技能页新增受控专家提示词目录、预览、编辑与恢复内置基线；REST 契约见 §3.6.2。会话选择器仍不返回专家提示词或工具视野。
...
### 3.6.2 Agent 技能与 Prompt 编排（V1.58 起，V1.97 增量）
...
返回技能页所需的专家目录 `{ "items": [...] }`。每项只有 `id`、`name`、`description`、`badge`、`prompt_available` 与 `overridden`；**不返回**提示词正文或工具视野。默认专家没有专属提示词时 `prompt_available=false`，前端只展示说明，不提供编辑入口。

#### `GET /api/admin/experts/{expert_id}/prompt`

仅可读取声明专属提示词的内置专家。返回 `{ "expert_id", "name", "content", "builtin_content", "revision", "overridden" }`：`content` 是本回合实际注入的有效文本，`builtin_content` 是随代码分发的只读基线，`revision` 为有效文本的 16 位修订指纹。未知专家返回 `NOT_FOUND`；无专属提示词的默认专家返回 `VALIDATION`。

#### `PUT /api/admin/experts/{expert_id}/prompt`

body：`{ "content":"...", "expected_revision":"16位修订指纹" }`。服务端仅把与内置基线不同的有效文本按专家 ID 保存为覆盖层；提交内置基线即删除覆盖并恢复发布版本。文本上限 30,000 字符，疑似密钥字段或接管性措辞返回 `VALIDATION`，修订不一致返回 `CONCURRENCY`。每次写入记录 `agent_expert_prompt_update` 审计，明细仅含专家 ID、字符数、修订与是否覆盖，绝不保存正文；运行时按「核心 → 专家 → 协议档补充」顺序装配，专家提示词不能扩大工具范围或覆盖核心边界。

V1.97：首次创建配置与后续修改统一在数据库行锁内读取最新修订、校验和写入，覆盖层与审计同事务提交；失败全部回滚，成功响应必须对应已写入内容。配置正文仍存储于受控设置，**仅审计不记录正文**。兼容早期 JSON 字符串形式的映射；损坏配置返回 `INTERNAL`，不得静默回落基线或丢弃其他专家配置。编辑器的「恢复内置」只替换草稿，点击保存后才生效；加载中与保存中禁止重复提交，旧读取响应不得覆盖当前专家草稿。
...
**V1.96（2026-09-14）— 专家提示词受控管理**

`GET /api/admin/experts` 仅返回专家展示目录，`GET/PUT /api/admin/experts/{expert_id}/prompt`
提供有专属提示词专家的有效文本预览和受控修改。有效文本由内置基线与按专家 ID 的
数据库覆盖层组成；恢复基线会删除覆盖层而不修改镜像中的发布文件。保存采用 16 位修订
指纹防止并发覆盖，并按既有提示词安全规则拒绝疑似密钥字段和接管性措辞。每次成功写入
都记录最小审计信息，正文不会进入审计日志、WebSocket 事件或专家选择器响应。

| 实际修改文件 | 作用 |
| --- | --- |
| `backend/api/app/expert_prompt_settings.py` | 专家覆盖层持久化、有效文本解析、修订指纹与安全校验。 |
| `backend/api/app/agent/{experts.py,loop_wiring.py}` | 严格管理端专家寻址；每回合从覆盖层装配专家系统段。 |
| `backend/api/app/routers/admin.py` | 专家目录与提示词预览/编辑接口、最小审计。 |
| `frontend/src/{views/AdminProfiles.vue,components/modals/AgentExpertPromptModal.vue,api/http.ts,api/types.ts}` | Agent 技能页的专家提示词卡片、预览/编辑/恢复交互与接口契约。 |
| `backend/api/tests/{test_agent_experts.py,test_agent_skill_admin.py}` | 覆盖覆盖层生命周期、安全/并发门禁与未登录访问拒绝。 |

**V1.97（2026-09-14）— 专家提示词一致性审查修复**

修改代码文件与作用清单：`backend/api/app/expert_prompt_settings.py` 使用幂等建行、行锁、最新快照与显式 flush，兼容历史 JSON 字符串并拒绝损坏映射；`routers/admin.py` 将配置、审计、提交异常统一纳入回滚范围；`tests/test_agent_experts.py` 改用关闭 autoflush 的真实 SQLite Session 验证保存/恢复/审计/回滚；`tests/test_expert_prompt_concurrency_pg.py` 提供独立 PostgreSQL 库的首次并发与多专家防丢失验证；`tests/test_loop_wiring.py` 验证新配置仅影响下一回合；前端编辑器使用 `composables/useExpertPromptEditor.ts` 隔离请求代次，`tests/expertPromptEditor.test.mjs` 覆盖乱序、重复提交、草稿保护与恢复操作。技能导出保留原数组格式，Mock 提示词在页面生命周期内保持读写一致。
