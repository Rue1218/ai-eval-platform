...
> 本文描述的是仓库当前的 **Harness Engineering（驾驭工程）**，并明确区分已实现、受控边界和预留能力。接口字段与路径仍以 `AI测试与评估平台-API.md` 为准；产品范围与状态机以 PRD 为准。
...
| `backend/api/app/agent/harness.py` | 单回合编排、异步调度、确认卡、确认回执、`/stop` |
...
| `backend/api/app/agent/react.py` | 多轮 Think-Act-Observe、工具观察、proposed spec 组装 |
