...
| 4 | [阶段4-记忆层接入](docs/AI测试与评估平台-Harness阶段4-记忆层接入.md) V1.7 | `fix/memory-activation` | MemoryPort + Redis/pgvector；不接 LightRAG | 阶段 4.2 已补 pgvector 知识表/ACL Port 与真实 Redis/PG CI 服务测试；当前 PR 必须通过该服务门禁，且未授权扩充 forget REST/WS，**阶段 4 未验收** |
...
V1.11：阶段 3 评论修复已随 PR #73 合入 `main`；阶段 4.1 的首批 P1/P2 和独立审查两个 P1 均已在 `fix/memory-activation` 修复。阶段 4.2 新增同库 pgvector 知识表、向量 ACL/撤权 Port、运行时组合装配和 PostgreSQL/Redis CI 服务测试；本机 392 项 pytest 通过（容器用例因无 Docker 跳过），真实服务结果以当前 PR CI 为准。forget 仍无对外产品入口，阶段 4 不构成验收。

| 文件 | 作用 |
| :--- | :--- |
| `docs/AI测试与评估平台-Harness分阶段实施总册.md` | V1.11：同步阶段 4.2 pgvector 与容器 CI 门禁状态 |
| `docs/AI测试与评估平台-Harness阶段3-并行与流式收尾.md` | V1.6：记录四项评论修复已合入 `main` |
| `docs/AI测试与评估平台-Harness阶段4-记忆层接入.md` | V1.7：记录阶段 4.2 实现、容器验收与产品入口边界 |
