...
V1.10：阶段 3 评论修复已随 PR #73 合入 `main`；阶段 4.1 的首批 4 个 P1、1 个 P2 与独立审查发现的两个 P1 均已在 `fix/memory-activation` 修复，并覆盖 PG 对话归档活路径、消息角色、归属 ACL（含写入侧真实 owner 复核）、Redis 依赖/撤权容错、`/compact` 边界及提交后短期写入。pgvector 与真实 Redis/PG 容器联调仍未完成，阶段 4 不构成验收。
