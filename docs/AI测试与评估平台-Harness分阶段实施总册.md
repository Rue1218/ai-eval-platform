...
| 3 | [阶段3-并行与流式收尾](docs/AI测试与评估平台-Harness阶段3-并行与流式收尾.md) V1.5 | 暂缓，不进入当前阶段 0/1 活路径 | 批次并行能力、流式终态 | 当前分支按第十二章止血：并发门面、批次归并和流式收尾保持空壳，待阶段 3 单独 PR |
...
V1.9：按第十二章执行紧急止血。阶段 0/1 保留单调用活路径；阶段 3 的批次解析、`ParallelFacade`、`merge_batch`、`FINALIZING_STREAM` 从当前活路径撤下并留待独立 PR；MCP Transport、security、Alembic、Redis/pgvector、确认卡与规划模块均未改动。

| 文件 | 作用 |
| :--- | :--- |
| `docs/AI测试与评估平台-Harness分阶段实施总册.md` | V1.9：记录阶段 3 暂缓与当前分支止血边界 |
