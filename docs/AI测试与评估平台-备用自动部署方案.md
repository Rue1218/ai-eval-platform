...
## 10. V1.5 增量部署与额度治理（2026-09-14）

CI 和 Deploy 的 push 入口均忽略 `docs/**`、根目录 `*.md`、各级 `README.md`，PR 不触发 Actions。纯说明文档只做内容及差异检查，不执行应用构建。镜像输入中的运行时提示词（如 `backend/media_mcp/app/SKILL.md`）仍需正常构建与发布。

构建计划统一由 `deploy/plan_services.py` 计算，Actions 输出矩阵，服务器使用相同脚本生成构建和配置应用列表。对比基准始终是上次成功部署提交，保留失败轮次的累计变更；首次部署或基准不存在时全量初始化。

- 单改 API 或一个 MCP 工具，仅重建对应服务，runner 不再每轮构建。
- 工具以 `backend/*_mcp/Dockerfile` 自动发现，Compose 名称遵循 `<名称>-mcp`，新增工具仍须登记 Compose 服务。
- COPY/ADD 引用的共享包变动会选择所有消费者，包括新增 MCP；删除和移动文件同样检测。动态或无法可靠解析的 Dockerfile 输入保守按整个上下文处理。
- 部署脚本变化只应用配置、复用已有不可变镜像；Compose 可能改变构建参数或上下文，因此仍保守全量构建。暂不解析 `.dockerignore` 排除项，可能多构建，但不会因此漏构建。
- 服务器无运行时差异时在 Docker 校验、拉取和清理之前退出。运维配置应用列表与构建列表分开，复用镜像时不强制重建容器。
- 备用巡检发现自成功版本以来只有说明文档变化时直接跳过，不再等待不会触发的 Actions；不推进真实上线标记，保留后续累计差异。
- 部署串行且不取消进行中的容器切换；缺少部署凭据时标记失败，避免跳过部署却推进成功基准。手动 Deploy 限制在 `main`。

预期收益来自减少无关镜像构建与拉取；实际耗时需后续生产发布测量，本次不声明固定百分比加速。

### 修改代码文件与作用清单（V1.5）

| 文件 | 作用 |
| :--- | :--- |
| `AGENTS.md` | 明确纯文档免构建、部署变更的专项门禁和共享部署计划约定 |
| `.github/workflows/ci.yml` | 移除 PR 触发并跳过纯说明文档 |
| `.github/workflows/deploy.yml` | 使用共享矩阵、跳过空部署、串行发布及成功基准保护 |
| `deploy/plan_services.py` | 基于 Git 差异与 Dockerfile 输入动态选择业务/MCP 镜像 |
| `deploy/deploy.sh` | 共用构建计划、文档提前退出、分离镜像构建与配置应用 |
| `deploy/auto-deploy-watch.sh` | 说明文档巡检直接跳过，保留真实成功基准 |
| `deploy/tests/test_plan_services.py` | 真实临时 Git 仓库覆盖新增工具、共享依赖、移动文件及累计改动 |
| `deploy/tests/test_build_artifacts.py` | 补齐 Windows Git Bash 的 coreutils 搜索路径 |
