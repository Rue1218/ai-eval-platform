...
| 对应后端计划 | V1.5（契约提供方） |
| 撰写日期 | 2026-08-17 |
| 最近修订 | 2026-08-24：补齐 AdminProfiles 工具清单接口状态、空清单/错误态和浏览器只读边界；修复 Agent 附件输入框内布局、用户消息附件位序、历史预览元数据和供应商 Logo；2026-08-24：统一供应商 Logo 为可核验的官方品牌图形，产品级 Logo 无独立矢量资源时使用文字标记；2026-08-23：对齐 LangGraph 单轮 Agent 与 WS 首期事件边界；2026-08-19：M1 页面全部对接真实 API（含流式思考卡/打字机渲染）；M2 数据集/用例/报告页提前落地 |
| 计划起点 | 2026-08-18 |
