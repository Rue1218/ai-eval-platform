...
| 文档版本 | V1.8 |
| 对应 PRD | V1.10（功能唯一权威） |
| 对应 API | V1.24（路径与 JSON 契约唯一权威） |
| 对应规范 | AGENTS.md V1.0（AI 行为准则、全中文注释、Naive UI 主题令牌与构建门禁） |
| 对应设计规范 | V1.2（页面 / 组件 / 浮层 / 令牌） |
| 对应总计划 | V1.5（日历与门禁） |
| 对应后端计划 | V1.5（契约提供方） |
| 撰写日期 | 2026-08-17 |
| 最近修订 | 2026-09-01：工作区管理页增加可选勾选、当前列表全选和批量清理；2026-08-24：补齐 AdminProfiles 工具清单接口状态、空清单/错误态和浏览器只读边界；修复 Agent 附件输入框内布局、用户消息附件位序、历史预览元数据和供应商 Logo；2026-08-24：统一供应商 Logo 为可核验的官方品牌图形，产品级 Logo 无独立矢量资源时使用文字标记；2026-08-23：对齐 LangGraph 单轮 Agent 与 WS 首期事件边界；2026-08-19：M1 页面全部对接真实 API（含流式思考卡/打字机渲染）；M2 数据集/用例/报告页提前落地 |
| 计划起点 | 2026-08-18 |
...
- [x] `/login` `/agent` `/tasks` `/admin/users` `/admin/profiles` 主路径 + 空态 + 错误码
- [x] WS 短票、心跳、补发
- [x] 确认卡未 ack 无任务
- [x] 全部成员看见相同导航；非创建者只能看到不可用的取消说明
- [x] 无 TestPilot 字样、无 T logo、无旧胶囊

**2026-08-19 实际进度备注（V1.4）**：M1 出口五项已全部达成；另提前交付——Agent 流式回复/思考卡/打字机兜底、会话删除、调度中心可视化（星图/工作流编排）、数据集与用例工作台结构还原（对接真实接口）、报告页接真实数据。M2 剩余：报告对比基线 Δ、样本明细页接 `GET /api/reports/{id}/samples`（后端已就绪）、表单双入口联调（W9）。
...
### 11.9 供应商 Logo 对齐修订（2026-08-24）

供应商图标优先使用 Simple Icons 或供应商官网公开品牌资源；无法验证产品级独立图形时，不再借用母公司 Logo，改用带供应商语义的文字标记。`/admin/profiles` 协议档列表与 `/agent` 模型选择器、消息头统一复用 `ProviderLogo`，确保同一供应商在两个页面显示一致。

| 文件 | 作用 |
| --- | --- |
| `frontend/src/components/ProviderLogo.vue` | 修正 StepFun、SiliconFlow、Groq 图形，移除 Doubao/Qianfan 的 ByteDance/Baidu 误用，并支持不同品牌图形的 viewBox 与多路径渲染 |
| `frontend/src/utils/providerLogo.ts` | 保持模型、名称和端点到供应商 Logo key 的统一推断 |

### 11.10 Agent 附件交互与历史预览修订（2026-08-24）

附件添加按钮缩小并放入输入内容区左侧；待发送附件预览留在输入框内部，用户消息中的附件卡片置于提示词上方。历史消息消费服务端补齐的 `filename`、`size`、`content_type` 与 `content_url`，避免刷新后退化为裸文件 ID；模型上下文解析属于后端 Agent 链路，前端只负责上传、预览和回显。

| 文件 | 作用 |
| --- | --- |
| `frontend/src/views/Agent.vue` | 调整附件按钮、待发送附件和用户消息附件的布局与位序 |
| `frontend/src/components/agent/AttachmentPreview.vue` | 为消息/输入框附件提供更紧凑的缩略卡片尺寸 |
| `frontend/src/api/types.ts` | 增加历史附件安全元数据类型 |
| `frontend/src/components/ProviderLogo.vue` | StepFun 紧凑图标对齐参考图二的黑色阶梯标记 |

### 11.11 AdminProfiles 工具清单检查修订（2026-08-24）

协议档页的 MCP Tab 仅通过同源 `GET /api/mcp/tools` 读取受控清单，不在浏览器直接执行内部 ToolCall。页面区分未检查、接口失败、接口成功但返回空清单和已加载清单，避免把 HTTP 200 或空响应误报为“全部工具就绪”；同时移除重复触发的清单请求和浏览器不可达的容器内 API 地址。

| 文件 | 作用 |
| --- | --- |
| `frontend/src/views/AdminProfiles.vue` | 增加工具清单请求状态、错误/空态和真实延迟展示；将 MCP 宿主说明调整为浏览器只读检查，并避免切换 Tab 重复请求 |
| `frontend/src/components/modals/McpToolModal.vue` | 将“ToolCall 实时执行”文案改为同源接口检查，明确浏览器不绕过边界调用内部工具 |

### 11.12 工作区批量清理交互修订（2026-09-01）

工作区管理页支持对当前列表中的会话沙箱文件夹和孤立文件夹进行可选勾选；提供逐区全选、页面级全选和批量清理确认，清理过程复用既有删除接口并反馈全部成功、部分成功或全部失败结果。分页、筛选和刷新会清空旧选择，避免对不可见或已过期条目误操作。

| 文件 | 作用 |
| --- | --- |
| `frontend/src/views/AdminWorkspaces.vue` | 增加工作区/孤立文件夹选择列、全选控件、批量清理确认、并发结果统计和窄屏换行样式 |
