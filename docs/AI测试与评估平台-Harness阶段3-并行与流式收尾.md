...
| 当前验收状态 | **审查评论已修复并合入 `main`**：4 项（3 个 P1、1 个 P2）随 PR #73 合入；阶段 4 仍须独立验收 |
...
本修复不新增 REST/WS 字段、不改变默认串行开关、不增加迁移或存储依赖。四项已随 PR #73 审查并合入 `main`，本阶段的评论债务关闭。
...
§7.1 的代码与定向回归已随 PR #73 合入，阶段 3 审查债务关闭。阶段 4 已按用户授权受控启动，但必须独立验收，且不得改并行门禁与 WS 字段。

---

## 修改代码文件与作用清单

V1.6：记录阶段 3 的 3 个 P1、1 个 P2 修复已随 PR #73 合入 `main`。默认串行批次复用批次归并；最终交付状态对齐消息 emit/持久化边界；下一轮 observation 保留内部 `ErrorClass`；参数绑定失败也持久化 child span。无 REST/WS 契约扩展、无迁移、无新增依赖。
