...
> 版本：V0.6 ｜ 审查日期：2026-09-16 ｜ 状态：基准方法与榜单建设提案；文本 benchmark、P1 调度和三类准备草稿交付已实现，正式套件/计划、完整制品修订与榜单仍未实施。
>
> 目标：回答基准测试测什么、怎么测、需要什么题目和用例、Agent 做什么、裁判如何评分、标准如何制定、报告如何生成，以及如何形成可信榜单，并修正常见概念混淆。
>
> 本次实施基线：`c29de49`；准备阶段增量契约为 PRD V1.31、API V2.9，完整计划/报告设计仍见 PRD V1.30、API V2.8。最初方法调研基线为 `3ac0ce738f4b551510fa5a9212ab0164239390ef`。本批未对真实模型发起评测，所有计算示例均为说明性数据。
>
...
当前保留 `general`、`testcase-agent`，新增 `benchmark-designer`、`benchmark-data-curator`、`benchmark-scoring-designer`。六工具中的 `agent.spawn` 已支持同会话准备成果的精确引用；三类 JSON 草稿由服务端校验后生成 `expert_deliverable.v1`。蓝图先完成，数据/评分草案可并行；结构通过不表示事实验真、数据发布或正式评分。完整修订链、报告专家和计划冻结入队未实现。边界见[专家协作方案 §13.8–§13.11](AI测试与评估平台-专家Subagent与多Agent协作技术方案.md)及 API V2.9。
...
| 专家协作组织 | P1 六工具及三类准备专家、结构化草稿和同会话引用已实现；完整修订、正式资产/报告与计划桥接待实现 |
...
### V0.6 修改代码文件与作用清单（2026-09-16）

同步准备专家首批能力，代码清单见[专家协作方案 V0.9 修改清单](AI测试与评估平台-专家Subagent与多Agent协作技术方案.md)。新增三角色、结构化草稿与固定引用；基准执行器、正式裁判、冻结计划和统计公式未改变。真实供应商联合试点、正式资产/报告接入、J2/J3 与榜单建设继续待实施。
