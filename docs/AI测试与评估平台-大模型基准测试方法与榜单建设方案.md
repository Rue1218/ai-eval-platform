...
> 版本：V0.4 ｜ 审查日期：2026-09-15 ｜ 状态：方法调研与产品技术提案，未实施。
>
> 目标：回答基准测试测什么、怎么测、需要什么题目和用例、Agent 做什么、裁判如何评分、标准如何制定、报告如何生成，以及如何形成可信榜单，并修正常见概念混淆。
>
> 本地核对基线：`3ac0ce738f4b551510fa5a9212ab0164239390ef`；PRD V1.26、API V2.4。本文核对了代码和公开原始资料，未对任何真实模型发起评测，所有计算示例均为说明性数据。
>
...
| 数据/需求专家 | 候选题与来源核对、覆盖检查 | 未审核题不能进入正式测试集 |
| 用例专家 | 构造边界、多轮和执行用例 | 不能替目标模型作答并计入目标成绩 |
| 执行分析专家 | 看执行事实、定位环境/适配问题 | 不凭文本声称任务已执行 |
| 评分复核专家 | 核对分歧与证据 | 初评前不能向裁判灌输期待结论 |
| 报告专家 | 从指标事实表解释优缺点 | 不自行计算或编造排名、区间、价格 |
...
| 文件 | 作用 |
| --- | --- |
| `docs/AI测试与评估平台-大模型基准测试方法与榜单建设方案.md` | 端到端方法方案；V0.4 补齐通用业务数据来源和 evaluation_asset_manifest.v1 的套件/计划交接与验收 |
| `docs/AI测试与评估平台-专家Subagent与多Agent协作技术方案.md` | 同步运行契约修复、judge_policy.v2 与预算保障和验收边界 |
| `docs/AI测试与评估平台-测试数据集与黄金集采集技术方案.md` | V2.5 增加通用业务任务生成/验真/发布与统一任务交付契约；延续 V2.4 证据和评分边界 |

没有修改业务代码、已有报告口径、PRD/API 或数据库；没有生成真实模型成绩或发布排行榜。
