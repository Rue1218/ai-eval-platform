...
| 对应开发计划 | V1.5 |
| 撰写日期 | 2026-08-17 |
| 最近修订 | 2026-08-23：Agent 对话助手消息增加供应商 Logo、模型名、协议档名和时间；2026-08-23：Agent 输入框模型选择器增加供应商 Logo；2026-08-23：协议档供应商卡片改用本地品牌标识并补充 Gemini 归类；2026-08-23：对齐 LangGraph 单轮 Agent、WS 基础事件和 Harness 冻结边界；2026-08-21：协议档增加 Embedding / Reranker 独立 URL、模型 ID、Key 配置，仍按 profile 环境文件隔离持久化 |
...
## 2. 角色与壳层权限（对齐 PRD 2.1 单一角色「成员」· 全员同权）

平台采用**单一角色「成员」（全员同权）**架构：

| 壳层能力 | 权限规范与交互反馈 |
| --- | --- |
| 登录、查看各类型评测报告、访问 7 天免登录分享链接 | 允许 |
| Agent 对话、创建/执行评测任务、取消与重新发起任务 | 允许；创建与取消均写操作审计 |
| 数据集/用例/知识库管理（文件树组织、自定义列、单元格即点即改、AI 合成与补全） | 允许；重要变更记录版本快照 |
| 协议档维护、连通性检查、压测治理白名单与预算配置 | 允许；敏感参数变更写审计 |
| 成员开户、停用账号、重置密码 | 允许；强制系统级保护：不可停用系统中最后一名正常账号 |

侧栏全量渲染所有工作台与治理入口。登录：HttpOnly Cookie 12h 可续；密码 ≥8 位含字母和数字；首次登录必须改密。WebSocket 只用 5 分钟短票，见 §6。
...
## 17. 调度中心组件设计规范（Dispatch Topology & Node Governance）

| 组件 | 结构与视觉类名 | 交互与动态规范 |
| --- | --- | --- |
| **调度内核雷达** | `.radar` + `.radar-core` + `.radar-blip` | 绿色同心环扫描光束；光斑（blip）按心跳周期（500ms）随机闪烁，代表活跃 Worker 节点探测 |
| **实时分发拓扑** | `.topo` + `<svg class="wires">` + `.wire.live` | 待分发队列（左）到目标 Worker（右）之间绘制三次贝塞尔曲线（`M x1 y1 C mx y1, mx y2, x2 y2`）；带流光动画 |
| **Worker 节点卡片** | `.agent-node` (`.idle` / `.busy` / `.offline` / `.draining`) | 顶部节点名与状态徽章；中部专精能力标签（`benchmark`, `rag`, `testcase`, `stress`）；双指标条（CPU 负载 / RAM 显存占用）；底部活跃任务槽位 |
| **节点治理弹窗** | `AE.modal` / `n-modal` | 点击 Worker 节点弹出；支持切换运行状态（上线/排水/下线）、调节调度权重（Weight 10~500）、维护专精标签、一键软重启 |
| **AI 调度优化卡** | `.ai-card` + `.ai-badge` + `#ai-advice` | 基于实时队列深度与节点倾斜度输出建议；支持「采纳建议」一键动态应用分发策略与并发容量 |

## 18. 评测报告中心设计规范（Report Center UI Specifications）

| 报告类型 | 核心模块与视觉类名 | 交互与图表规范 |
| --- | --- | --- |
| **Benchmark 报告** | `.baseline-banner` + `.delta-badge` + `.radar-container` + `.judge-breakdown-grid` + `#sample-table` | 多模型并排对比，雷达图展示 4 维综合能力（contain / 低失败率 / 低延迟 / Judge）；Judge 裁判给出 1~5 分维度拆解与评语；样本表格支持「全部 / 仅分歧 / 仅失败」过滤，支持展开 HTTP Raw 报文与错误码排查 |
| **RAG 评测报告** | `.rc-bars` + `.retrieval-chunk` + `.status-tag-err` | LightRAG 4 种检索模式（naive / local / global / hybrid）在 Hit Rate@K、MRR、Recall@K、答案 contain 上的水平柱状条对比；退化 ≥5pp 标红；Top-K 相似度召回切块高亮命中态 |
...
## 本次修订代码文件与作用清单（2026-08-23 · Agent 助手消息头）

| 文件 | 作用 |
| --- | --- |
| `frontend/src/components/ProviderLogo.vue` | 使用无边框的本地供应商图形；Gemini 使用 Google 风格多色渐变标识 |
| `frontend/src/views/Agent.vue` | 在助手消息中显示供应商 Logo、模型名称、协议档名称和 `MM/DD HH:mm` 时间，并为历史消息与流式消息补齐元数据 |

---

*V1.6：在 Agent 对话助手消息头中补充供应商 Logo、模型名称、协议档名称和时间，并复用本地品牌标识。产品以 PRD 为准。*
