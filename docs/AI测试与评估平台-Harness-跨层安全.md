...
| 上游权威 | Harness 需求文档 V1.4.4 §4.2/§4.4/§4.5/§5.2.1/§7；API.md V1.4+ §4.4/§5；PRD §5.1.2 |
...
| 文件 | 操作 | 作用 |
| :--- | :--- | :--- |
| `docs/AI测试与评估平台-Harness-跨层安全.md` | 新增 V0.3 | M8 跨层安全模块设计：定义递归脱敏（`redact`/`redact_for_log`，5 类键）与确认卡 owner 校验 + 行锁（`assert_confirm_owner`/`lock_pending_confirm`）；含接口签名级与 TDD 验收；明确与通用 `security.py`（用户鉴权）的边界。 |
