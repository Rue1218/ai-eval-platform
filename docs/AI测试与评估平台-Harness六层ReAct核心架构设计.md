...
| 版本 | V1.10（目标架构 + 阶段 0–4 需求分析索引 + 首期覆盖矩阵） |
| 审查日期 | 2026-08-22 |
...
> **当前模型层修订（2026-08-22）**：大模型调用正文已独立到 `backend/api/app/llm/`，该层不再反向依赖 `app.harness` 或 `app.agent`；`backend/api/app/harness/llm/` 仅保留兼容导出。本文关于 `harness/llm` 为模型正文归属的历史迁移描述以该修订为准，ReAct/确认/事件/Worker 仍属于上层 Harness 业务编排。
...
> **V1.10 定向修订定位**：阶段 3 审查提出的默认串行批次归并、最终交付状态时序、内部 ErrorClass 回填与参数失败 span 审计均已在独立修复分支完成，并补充定向回归；待 PR 复审与合入。阶段 4 的独立验收状态不因此改变。
...
权威展开见 [`docs/AI测试与评估平台-Harness阶段3-并行与流式收尾.md`](docs/AI测试与评估平台-Harness阶段3-并行与流式收尾.md) V1.5。默认串行批次归并、最终交付状态机、ErrorClass observation 回填及参数失败 span 审计已在 `fix/harness-stage3-comments` 修复并通过定向回归；在修复 PR 复审合入前，阶段 3 仍不得按 `main` 已验收表述。阶段 4 已获用户明确授权受控启动，但必须独立验收。
...
## 第十一章：阶段 4 需求分析索引（V1.9）

权威展开见 [`docs/AI测试与评估平台-Harness阶段4-记忆层接入.md`](docs/AI测试与评估平台-Harness阶段4-记忆层接入.md) V1.4。当前仅实现内存/组合 Port、Redis 适配器骨架和纯 Context 基础组件；尚未接入 `app/agent/` 活路径，未实现 PG/pgvector/Alembic，且存在消息角色、记录归属 ACL、Redis 依赖与撤权容错的待修项，因此不构成阶段验收。
...
- [x] 阶段 2–4 是否具备同样五块分析？是；第九至十一章为索引，展开见阶段 2 V1.3、阶段 3 V1.5、阶段 4 V1.4。
- [x] 首期五阶段是否覆盖目标架构全量？否；第十二章给出覆盖矩阵，MCP Transport / file_sandbox / security/ / 长任务控制面 / LightRAG 为挂起项，禁止在阶段 1–4 顺手实现。

## 本次文档变更范围

V1.10：同步阶段 3 评论修复状态。默认串行批次和并行批次统一走 `merge_batch`，最终交付状态移到消息 emit/持久化边界，observation 保留受控 `ErrorClass`，参数失败 child span 也可审计。该修复待 PR 复审合入；阶段 4 状态、对外协议和首期范围均未改变。

| 文件 | 作用 |
| :--- | :--- |
| `docs/AI测试与评估平台-Harness六层ReAct核心架构设计.md` | V1.10：阶段 3 评论修复状态与合入门禁 |
| `docs/AI测试与评估平台-Harness分阶段实施总册.md` | V1.8：阶段 3 修复分支进度与 PR 复审门禁 |
...
| `docs/AI测试与评估平台-Harness阶段3-并行与流式收尾.md` | V1.5：四项评论修复、定向回归与 PR 复审条件 |
| `docs/AI测试与评估平台-Harness阶段4-记忆层接入.md` | V1.4：同步阶段 3 修复 PR 状态；第一批实现、未完成项和自身阻塞项不变 |
