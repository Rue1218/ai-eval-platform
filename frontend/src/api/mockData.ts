...
export const MOCK_MCP_TOOLS: McpTool[] = [
  ...
  {
    name: 'task', display_name: '任务规划', desc: '仅为复杂多步骤工作维护完整执行清单；简单单步任务不调用。每次提交完整 steps，并及时更新 pending/in_progress/completed 状态',
    permission: 'task.plan', enabled: true, source: 'builtin',
    transport: 'native', category: 'native_toolcall', server_id: 'platform.native', risk_level: 'read',
    ...
    parameters_schema: {
      ...
      properties: {
        description: { type: 'string', minLength: 1, maxLength: 80, description: '本轮任务目标的简短概括' },
        prompt: { type: 'string', minLength: 1, maxLength: 2000, description: '兼容旧调用的完整目标；新调用以 description 为准' },
        steps: {
          type: 'array',
          minItems: 1,
          maxItems: 12,
    ...
    code_details: {
      ...
      code_summary: '校验完整 steps 清单 -> 整表替换会话内规划 -> 将真实步骤安全投影给任务抽屉',
    },
    pipeline: {
      stages: [
        { step: 1, name: '执行清单步骤解析', desc: '读取完整 steps 步骤数组' },
        { step: 2, name: '状态机跃迁合规', desc: '校验 pending -> in_progress -> completed' },
        { step: 3, name: '会话规划替换', desc: '以本次完整清单替换旧规划，不创建 PG 任务' },
        { step: 4, name: '前端可视化同步', desc: '通过持久 ToolCall 展示投影更新任务抽屉' },
      ],
    },
  },
