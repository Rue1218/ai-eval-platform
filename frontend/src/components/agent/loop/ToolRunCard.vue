<template>
  <details class="tool-run" :class="tool.status" :open="interactions.some(i => !i.resolved)">
    ...
    <div class="tool-body">
      ...
      <section v-if="tool.event === 'tool.result'"><h4>结果 <span v-if="tool.exit_code !== undefined">· 退出码 {{ tool.exit_code }}</span></h4><MediaResultPreview v-if="mediaPreview" :preview="mediaPreview"/><details v-if="mediaPreview" class="raw-result"><summary>查看安全结果</summary><pre>{{ tool.display.result_preview }}</pre></details><details v-else-if="(tool.display.result_preview?.length || 0) > 2000"><summary>展开长输出</summary><pre>{{ tool.display.result_preview }}</pre></details><pre v-else>{{ tool.display.result_preview === '' ? '（空输出）' : tool.display.result_preview ?? '未提供结果预览' }}</pre></section>
...
import { mediaPreviewFor } from '../../../agent/loop/mediaPresentation'
...
const props = defineProps<{ tool: ToolRun; interactions: InteractionRecord[]; canControl: boolean; online: boolean }>()
const emit = defineEmits<{ respond: [InteractionRecord, Data] }>()
/** 媒体预览只读取已登记工具的安全 JSON 投影，原始结果仍可折叠查看。 */
const mediaPreview = computed(() => mediaPreviewFor(props.tool))
</script>
